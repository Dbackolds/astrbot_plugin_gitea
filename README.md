...
**示例**:
```
/gitea add https://gitea.example.com/user/myproject my_secret_key
```
...
**A**: 
1. 在目标 QQ 群组中执行命令：`/gitea add <repo_url> <secret>`
2. 插件会自动使用当前群组作为通知目标
3. 不需要手动输入群号
...
```
plugin_data/astrbot_plugin_gitea/monitors.json
```

每个监控配置包含：
- 仓库 URL
- Webhook 密钥
- 目标会话的 unified_msg_origin（自动获取）
- 创建时间
...
- 🎯 改进消息发送机制，使用 AstrBot 推荐的 unified_msg_origin 方式
- ✨ 简化添加配置流程，自动使用当前群组作为通知目标
- � 改修复数据持久化路径问题
- 📝 改进日志输出和错误提示
